...
import de.schlichtherle.util.NetAddresses;
...
public class LicenseManager implements LicenseCreator, LicenseVerifier {
    ...
    static {
        while(!LICENSE_SUFFIX.equals(LICENSE_SUFFIX.toLowerCase()))
        	throw new BeaconException("LICENSE_SUFFIX Exception!");// paranoid
    }
    ...
    private static final String EXC_LICENSE_IP_INVALID = new ObfuscatedString(new long[] {
    	0x8C475FB2DC06F525L, 0x5E35B13538A0BD95L, 0x9C1ACA13F7DB38B3L, 
    	0x9EC9179827C4875EL
    }).toString(); /* => "exc.ipAddressIsInvalid" */
    ...
    throws LicenseContentException {
        ...
        final String ip = content.getIp();
        if (!NetAddresses.isValid(ip))
            throw new LicenseContentException(EXC_LICENSE_IP_INVALID);           
    ...
    protected synchronized void validateAtCreate(final LicenseContent content)
    throws LicenseContentException {
        final LicenseParam param = getLicenseParam();
        if (!param.getSubject().equals(content.getSubject()))
            throw new LicenseContentException(EXC_INVALID_SUBJECT);
